...
export default function RecommendPage({ onPlayTrack, favorites, onToggleFavorite, userPlaylists, onAddToPlaylist }: RecommendPageProps) {
  ...
  const calculatePersonalizedMix = async () => {
    ...
    if (topGenres[0]) {
      ...
      const fallbackRanking = rankingVideos.length > 0 ? rankingVideos : await getRankingVideos(1);
      g1Results = fallbackRanking.slice(0, 10);
      setGenre1Tracks(g1Results);
    }
    ...
    if (topGenres[1]) {
      ...
      const fallbackLatest = latestVideos.length > 0 ? latestVideos : await getLatestVideos(1);
      g2Results = fallbackLatest.slice(0, 10);
      setGenre2Tracks(g2Results);
    }
  ...
  useEffect(() => {
    const loadInitialData = async () => {
      ...
      const ranking = await getRankingVideos(1);
      setRankingVideos(ranking);
    };
